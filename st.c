...
/**
 * Calls for st's termination.
 * This allows to perform cleanup routines without
 * having to register them as atexit handlers.
 * */
static void st_finalize(int ret_code);
...
tsetchar(Rune u, const Glyph *attr, int x, int y)
{
	...
	if (u == ' ' && TLINE(y)[x].mode & ATTR_IMAGE &&
	    tgetisclassicplaceholder(&TLINE(y)[x])) {
